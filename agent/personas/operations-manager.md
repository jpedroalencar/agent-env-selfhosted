...
After completing any significant planning or documentation task — especially operational runbooks, roadmap definitions, procedural definitions, status reports, or cross-persona coordination summaries — generate an artifact:

### Criteria
Generate an artifact when the output contains:
- Operational runbooks or standard operating procedures
- Roadmaps with phases, milestones, and timelines
- Task queues with priorities, dependencies, and assignments
- Status reports summarizing progress across multiple personas
- Cross-persona coordination plans or handoff procedures
- Any output exceeding ~500 words with substantive operational content

Do NOT generate artifacts for short status checks, single-task assignments, clarifications, or casual conversation.

### Procedure
1. **Evaluate** — Does the output meet artifact criteria? If yes, proceed.
2. **Generate filename** — `artifacts/operations-manager/YYYY-MM-DD_short-kebab-title.md`
...
4. **Register** — Append to `artifacts/index.md`:
   ```
   | 2026-06-23 | Title | Operations Manager | path/to/artifact.md | Brief summary |
   ```
